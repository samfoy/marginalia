...
13. **An empty `translation_index` is only complete when `build_attempted` is true** — zero entries is otherwise indistinguishable from a failed build, and accepting it blindly created a permanently stuck state (served from cache forever, never retried, device shows no translations). But rejecting *every* empty index was also wrong: a book with nothing to translate (Blood Meridian — English with a few Spanish phrases) then failed validation forever, so `/book-index/init` kept answering `needs_epub`, the device kept re-uploading, and the plugin surfaced **"Book Index upload failed"** after `MAX_EMPTY_RETRIES`. `_safe_translation_index()` stamps `build_attempted = True` on a **complete** build only (`skipped_candidates == 0`); `_translation_index_valid()` accepts an empty index only with that stamp. Keep both halves — dropping either one resurrects a loop.
14. **Each Book Index job owns its own upload file — never go back to a shared path.** Uploads are `{sha256}-{job_id}.epub` via `_publish_upload()`, torn down by `_cleanup_upload()`. History: they used to be shared and content-addressed (`{md5}.epub`), so every retry of one book hit the **same file**, and each job's `finally: os.unlink()` deleted the EPUB out from under the jobs still running — `FileNotFoundError` in the translation build → `translation_index: null` → the needs_epub loop in item 13. A shared path also had a **content-integrity** hole: a job opens its upload **twice** (`extract_epub`, then `_safe_translation_index`), so a colliding upload landing between those reads fed it a *different book's bytes* (verified with a forced-collision probe). Per-job files make that structurally impossible. Details that adversarial review forced and that must not be "simplified" away:
    - **SHA-256, not MD5**, for the upload filename, so the name isn't attacker-steerable. Correctness no longer *depends* on this (the job_id already isolates), but keep both — defence in depth.
    - Note MD5 is still used deliberately elsewhere: `epub_extract.file_hash` / `_epub_matches_cache` (existing cache keys) and KOReader's partial-MD5 (device edition binding). Those are compatibility contracts — changing them invalidates caches and breaks device matching.
    - Write to a temp file and `os.replace()` it in. Writing the destination directly **truncates a file a job is mid-read on**; a rename swaps the directory entry and existing readers keep their fd on the old inode.
    - The big write + fsync happens **outside** `_uploads_lock`; only the claim + rename are inside it. Holding the lock across a 100 MB write stalls every other upload.
    - A **failed `unlink` must not** be recorded in `_upload_done`, or every later cleanup returns early and the file leaks forever. Log it and leave it retryable.
    - `_upload_done` is a **bounded LRU** (`_UPLOAD_DONE_MAX`). This server is long-lived; an unbounded set grows one entry per upload forever.
    - The handler catches **any** exception from `_publish_upload` (not just `OSError`) and pops the job, or a failed publish leaves a job `pending` forever with no HTTP error.
    - `_mark_translation_build_complete()` requires `type(skipped) is int and skipped == 0`. `not index.get(...)` would treat a missing/None/malformed count as proof of completeness, letting a degraded index become permanently valid — and `True == 1`, so a bool must not pass either.
