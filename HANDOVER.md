...
14. **The uploaded EPUB path is SHARED, refcounted state — never unlink it unconditionally.** Uploads are stored content-addressed (`{md5}.epub`), so every retry of the same book resolves to the **same file**. Jobs used to `os.unlink(epub_path)` in `finally`, so the first job to finish deleted the EPUB out from under the jobs still running: their translation build died with `FileNotFoundError`, the record cached `translation_index: null`, and that fed the needs_epub loop in item 13. Use `_publish_upload()` (claims the path in the same lock that publishes it) and `_cleanup_upload()` (releases + unlinks atomically) — never write the destination in place and never unlink without checking the refcount. Details that adversarial review forced and that must not be "simplified" away:
    - Write to a temp file and `os.replace()` it in. Writing the destination directly **truncates a file an in-flight job is mid-read on**; a rename swaps the directory entry and existing readers keep their fd on the old inode.
